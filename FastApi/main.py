...
from sqlalchemy import (
    Column, Integer, String, Boolean, DateTime, Date, ForeignKey, Text, UniqueConstraint, cast, func, text
)
...
class LearnerProfileSchema(BaseModel):
    ...
    registered_on: Optional[datetime] = datetime.now(timezone.utc)
...
class LearnerTestBookingSchema(BaseModel):
    ...
    booking_date: Optional[datetime] = datetime.now(timezone.utc)
    license_code: Optional[str] = None
    registered_on: Optional[datetime] = datetime.now(timezone.utc)
...
def _serialize_datetime(dt):
    """Helper to safely convert datetime/date to ISO string"""
    if dt is None:
        return None
...
@app.get("/learner-test-bookings/pending/{date_str}")
def get_pending_learners(date_str: str, db: DBSession = Depends(get_db)):
    ...
    try:
        ...
        # Use string comparison with CAST to avoid psycopg3 type binding issues
        # Cast the date column to text and compare with date string
        pending_bookings = db.query(LearnerTestBooking).filter(
            func.cast(LearnerTestBooking.test_date, String) == date_str
        ).filter(
        ...
        for b in pending_bookings:
            try:
                result.append({
                    ...
                    "test_date": _serialize_datetime(b.test_date),
                    "result": b.result,
                    "license_code": b.license_code,
                    "booking_date": _serialize_datetime(b.booking_date),
                    "registered_on": _serialize_datetime(b.registered_on),
                })
            except Exception as e:
                # Log error but continue with other bookings
                import traceback
                print(f"Error serializing booking {b.booking_id}: {e}")
                traceback.print_exc()
                continue
...
def get_completed_learners(date: str, db: DBSession = Depends(get_db)):
    ...
    completed_bookings = db.query(LearnerTestBooking).filter(
        LearnerTestBooking.test_date == parsed_date,
        LearnerTestBooking.result != 'pending'
    ).all()
