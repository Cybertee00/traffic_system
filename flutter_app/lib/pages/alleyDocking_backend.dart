...
class Page3Backend extends ChangeNotifier {
  // List of 13 items (0 = green, 1 = red) representing circle colors
  List<int> circleColorData = [];
  
  // Rectangle color based on 14th digit (0 = green, 1 = red)
  ...
  static const int _aprilTagPort = 8766; // Alley Docking port for April Tag
  ...
  Page3Backend() {
    // Initialize with default values (4 zeros and 9 ones)
    circleColorData = [2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2];
    _previousStatusArray = [2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2]; // Initialize previous array to match
    updateCircleColors();
    notifyListeners();
  ...
  void connectToESP32() {
    ...
    try {
      ...
      debugPrint('Alley Docking - Connecting to ${_detectionSystem == DetectionSystem.aprilTag ? "April Tag" : "ESP32 Touch"} at $uri');
      ...
      _channel!.stream.listen(
        (data) {
          if (_enableWebSocket) {
            ...
            if (!_isConnected) {
              _isConnected = true;
              debugPrint('Alley Docking - Connected to ${_detectionSystem == DetectionSystem.aprilTag ? "April Tag" : "ESP32 Touch"}');
        ...
        onError: (error) {
          if (_enableWebSocket) {
            debugPrint('Alley Docking - WebSocket error: $error');
            _isConnected = false;
        ...
        onDone: () {
          if (_enableWebSocket) {
            debugPrint('Alley Docking - WebSocket connection closed');
            _isConnected = false;
            notifyListeners(); // Update UI to show disconnected status
            _scheduleReconnect();
          }
      ...
      if (_enableWebSocket) {
        debugPrint('Alley Docking - Failed to connect to Bridge: $e');
        _isConnected = false;
        notifyListeners(); // Update UI to show disconnected status
        _scheduleReconnect();
      }
    ...
    try {
      ...
      if (jsonData is List) {
        ...
        if (jsonData.containsKey('touchStates')) {
          ...
          // Single status value (not applicable for alley docking, but handle gracefully)
          return;
        } else {
          return;
        }
      ...
      // Ensure we have exactly 13 items for alley docking
      if (circleColorData.length != 13) {
        // Pad or truncate to 13 items
        while (circleColorData.length < 13) {
          circleColorData.add(0);
        }
        if (circleColorData.length > 13) {
          circleColorData = circleColorData.sublist(0, 13);
        }
      }
      
      // Update rectangle color based on 14th digit (if available in original format)
      if (jsonData is Map<String, dynamic> && jsonData.containsKey('touchStates') && (jsonData['touchStates'] as List).length >= 14) {
        final int fourteenthDigit = (jsonData['touchStates'] as List)[13] as int;
        rectangleColor = fourteenthDigit == 1 ? Colors.green : Colors.grey;
      }
  ...
  void _scheduleReconnect() {
    ...
    _reconnectTimer = Timer(const Duration(seconds: 5), () {
      if (!_isConnected && _enableWebSocket) {
        debugPrint('Alley Docking - Attempting to reconnect to ${_detectionSystem == DetectionSystem.aprilTag ? "April Tag Bridge" : "ESP32 Touch"}...');
        connectToESP32();
      }
    });
  }
  ...
  List<CircleData> circles = [
    ...
    CircleData(x: 0.20, y: 0.15, color: Colors.red),
    CircleData(x: 0.25, y: 0.30, color: Colors.red),
    CircleData(x: 0.25, y: 0.50, color: Colors.red),
    CircleData(x: 0.25, y: 0.70, color: Colors.red),
    CircleData(x: 0.25, y: 0.95, color: Colors.red),
    CircleData(x: 0.50, y: 0.95, color: Colors.red),
    CircleData(x: 0.75, y: 0.95, color: Colors.red),
    CircleData(x: 0.75, y: 0.70, color: Colors.red),
    CircleData(x: 0.75, y: 0.50, color: Colors.red),
    CircleData(x: 0.75, y: 0.30, color: Colors.red),
    CircleData(x: 0.80, y: 0.15, color: Colors.red),
    CircleData(x: 0.95, y: 0.05, color: Colors.red),
  ];
